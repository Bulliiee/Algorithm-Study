...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		int towerNum = Integer.parseInt(br.readLine());	// 타워 갯수 입력
		...
		for(int i = 0; i < towerNum; i++) {
			int currHeight = Integer.parseInt(st.nextToken());	// 체크할 타워 높이
			...
			while(!towers.isEmpty()) {
				...
				towers.pop();	// 위 if문에 안걸렸다면 현재 타워보다 작은애니까 pop해버림
			}
			...
			// 다음으로 넘어가면 현재 타워가 왼쪽이므로 푸시
			towers.push(new Tower(i+1, currHeight));
		}
